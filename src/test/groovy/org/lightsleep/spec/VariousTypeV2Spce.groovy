...
// VariousTypeV2Spce
@Unroll
class VariousTypeV2Spce extends Specification {
	...
	def "VariousTypeV2Spce various types"() {
	/**/DebugTrace.enter()
		...
		when:
			Transaction.execute(connectionSupplier) {
				if (various1 instanceof Various.PostgreSQL) {
					new Sql<>(Various.PostgreSQL.class).connection(it).delete(various1 as Various.PostgreSQL)
					new Sql<>(Various.PostgreSQL.class).connection(it).insert(various1 as Various.PostgreSQL)
				} else {
					new Sql<>(Various.class).connection(it).delete(various1)
					new Sql<>(Various.class).connection(it).insert(various1)
				}
			}
	...
	def "VariousTypeV2Spce number types"() {
	/**/DebugTrace.enter()
		...
		/**/DebugTrace.print('number types: insertion value', various1)
			...
			Transaction.execute(connectionSupplier) {
				new Sql<>(VariousInteger.class).connection(it).delete(various1)
				new Sql<>(VariousInteger.class).connection(it).insert(various1)
				various2 = new Sql<>(VariousInteger.class).connection(it)
					.where(various1)
					.select().orElseThrow({new NotFoundException()})
			}
	...
	def "VariousTypeV2Spce string type"() {
	/**/DebugTrace.enter()
		...
		when:
			...
			Transaction.execute(connectionSupplier) {
				new Sql<>(VariousString.class).connection(it).delete(various1)
				new Sql<>(VariousString.class).connection(it).insert(various1)
				various2 = new Sql<>(VariousString.class).connection(it)
					.where(various1)
